...
dojo.widget.defineWidget(
	...
	},
	{
		...
		// preFilterTextarea: Boolean
		//		whether to prefilter the contents of a textarea when initialising editor
		preFilterTextarea: false,
	 ...
	 *******/
		...
		open: function (/*DomNode, optional*/element) {
			...
			if(	(this.domNode["nodeName"])&&
				...
				if (this.preFilterTextarea) html=this._preFilterContent(html);
...
//			tmpContent.style.display = "none";
			...
			if(this.editNode){
				...
				setTimeout(ifrFunc, (this.easyEditClicked?666:250));
			}else{ // opera, safari
				ifrFunc();
			}
		},
...
// if moving with cursor keys & no selection, store our position (for KeyUp comparison).
//			if(!dojo.render.html.ie) {
				if(e.keyCode<41 && e.keyCode>36 && !e.shiftKey) {
					var sel_txt=dojo.withGlobal(this.window, "getSelectedText", dojo.html.selection);
 					this.oldrng=((sel_txt==null || sel_txt.length==0) ? this._createRange() : null);
				} else {this.oldrng=null;}
//			}
		},

		onKeyUp: function(e){
			// escape from tags at start or end of editnode using cursor keys... doesn't escape on IE7,
			// doesn't work with activeX -=Bruce
//			if(!dojo.render.html.ie) {
				if(this.oldrng && e.keyCode<41 && e.keyCode>36) {
					var newrng=this._createRange();

					// if cursor hasn't moved, try to 'escape' out to start or end of editNode.
					if ((dojo.render.html.ie && newrng.offsetLeft==this.oldrng.offsetLeft && newrng.offsetTop==this.oldrng.offsetTop) || (!dojo.render.html.ie && newrng.startContainer==this.oldrng.startContainer && 
						((e.keyCode<39 && newrng.startOffset==this.oldrng.startOffset) ||
						(newrng.endOffset==this.oldrng.endOffset)))) {
...
//			}
		...
		_initialFocus: true,
		onFocus: function(e){
			// summary: Fired on focus  -- initial focus now for all browsers
...
// 					break;
		...
		execCommand: function (/*String*/command, argument){
			...
			// fix up unlink in Mozilla to unlink the link and not just the selection
				...
				return returnValue;
			}else if((command == "inserttable")  //&& inserthtml supported
				){
				var cols = "<tr>";
...
//				setTimeout(function(){tr.select();}, 1);
				...
				//STOP 'indent' from making 'BLOCKQUOTE'!!
				if(command == "indent") {
					if(dojo.render.html.mozilla) {
					//<p> tags get set OK, but don't display indent in FF while editing!
						this.document.execCommand("useCSS", false, false);
						return this.document.execCommand(command, false, argument);
						this.document.execCommand("useCSS", false, true);
						return true;
					} else if(dojo.render.html.ie) {
		//indent only works properly in IE within a list, so we'll handle other cases
						if(!dojo.withGlobal(this.window, "getAncestorElement", dojo.html.selection,['ul','ol'])) {
							...
							if(node && node!=this.editNode) {
								var marginLeft=dojo.html.getStyleProperty(node,'margin-left');
								dojo.html.setStyle(node,'margin-left',
									(marginLeft.match(/ ?[0-9]+ ?px/)? parseInt(marginLeft)+40+'px' : '40px'));
							} else {
								//no format block, so set a formatblock 'div' and set margin on that...
				...
				//move out margin-left if set, otherwise let browser handle it.
				if(command == "outdent") {
					if(dojo.render.html.mozilla) {
						this.document.execCommand("useCSS", false, false);
						return this.document.execCommand(command, false, argument);
					...
					if(node && node!=this.editNode) {
						var marginLeft=dojo.html.getStyleProperty(node,'margin-left');
						marginLeft=(marginLeft.match(/ ?[0-9]+ ?px/)? Math.max(parseInt(marginLeft)-40,0):0);
						dojo.html.setStyle(node,'margin-left',(marginLeft? marginLeft+'px' : ''));
						return true;
					}
				}
				...
				//ie blockquote
				if(command=='formatblock' && argument.toLowerCase()=='<blockquote>') {
					command='indent';argument=null;}
		...
		queryCommandEnabled: function(/*String*/command){
			...
			if(command=='outdent') {return true;}// should be false if format block ancestor has margin-left of 0
if(command=='superscript' || command=='subscript') return true;
		...
		queryCommandState: function(command){
			...
			if(command=="createlink") {return(this.queryCommandEnabled("unlink"));}
...
// IE added to top innerhtml case because it was doubling-up DIVs with selectall insert method.
			}else if((this.window && this.window.getSelection && !dojo.render.html.moz) || dojo.render.html.ie){ // Safari, IE
