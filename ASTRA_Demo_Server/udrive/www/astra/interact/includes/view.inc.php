 ...
 *                    Kristian Koehntopp
 *
 * $Id: view.inc.php,v 1.2 2007/01/07 22:25:17 glendavies Exp $
 *
 */
...
class InteractView
{
  ...
  * @return    void
  */
  function InteractView($module_code='') {
    global $CONFIG;
  	if ($this->debug & 4) {
      echo "<p><b>Template:</b> root = $root, unknowns = $unknowns</p>\n";
    }
    $this->set_root($CONFIG['BASE_PATH'].'/templates');
    $this->set_unknowns('remove');
    $module_template = substr(basename($_SERVER['SCRIPT_NAME']),0,strlen(basename($_SERVER['SCRIPT_NAME']))-4);
   	$this->set_file(array(
		'header'		  => 'header.ihtml',
	...
	if (!empty($module_code)){
		$this->set_root($CONFIG['BASE_PATH'].'/modules/'.$module_code.'/templates'); 
		$this->set_file($module_code, $module_template.'.ihtml');
  	}
  }
  ...
  */
  function finish($str) {
	...
	switch ($this->unknowns) {
      ...
      case "remove":
        //$str = preg_replace('/{[^ \t\r\n}]+}/', "", $str);
		$str = preg_replace('/{[^J][A-Z_]+[1-9]?}/', "", $str);
		$str = preg_replace('/{[A-Za-z_]+Block}/', "", $str);
 ...
 /******************************************************************************
  * This function sets any undefined variables if they have a language file string
  * or a data value, or an error string
  *
  * usage: set_strings(array $module_strings, array $module_data array $errors)
  *
  * @param     $template		   name of tamplate to set variables for
  * @param     $module_strings     an array of strings for current module (required)
  * @param     $module_data        an array of data values for current module (optional)
  * @param     $errors             an array of error strings (optional)
  * @access    public
  * @return    true
  */
  function set_strings($template, &$module_strings, $module_data, $errors) {
	global $general_strings;
  	foreach($this->get_undefined($template) as $key=> $value) {
		if (isset($module_strings[strtolower($key)])){
			$this->set_var($key,$module_strings[strtolower($key)]);
		} else if (isset($general_strings[strtolower($key)])){
			$this->set_var($key,$general_strings[strtolower($key)]);
		} else if (strpos($key,'_VALUE')) {
			if (isset($module_data[strtolower(str_replace('_VALUE','',$key))])) {
				$this->set_var($value,$module_data[strtolower(str_replace('_VALUE','',$key))]);
			} else {
				$this->set_var($value,'');
			}
		} else if (strpos($key,'_ERROR')) {
			if (isset($errors[strtolower(str_replace('_ERROR','',$key))])) {
				$this->set_var($value,sprint_error($errors[strtolower(str_replace('_ERROR','',$key))]));
			}
		}
	}
	return true;
