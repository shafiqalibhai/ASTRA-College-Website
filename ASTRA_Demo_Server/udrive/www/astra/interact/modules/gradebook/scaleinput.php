...
* @version $Id: scaleinput.php,v 1.14 2007/07/30 01:57:00 glendavies Exp $
* 
*/
...
switch ($action) {
	...
	case add_scale:
		...
		$message = $gradebook->addScale($scale_name, $scale_description, $space_key, 'user');
...
$scale_sql = "SELECT name, scale_key FROM {$CONFIG['DB_PREFIX']}gradebook_scales WHERE (added_by_key='$current_user_key' OR space_key='$space_key') ORDER BY name";
...
if (!$action || $action=='add_scale' || $action=='modify_scale') {
	...
	$t->set_var('ADD_SCALE_LINK',"<a href=\"scaleinput.php?space_key=$space_key&module_key=$module_key\">".$gradebook_strings['add_scale'].'</a>');	
	...
	if (!$grade_action || $grade_action=='add_grade' || $grade_action=='modify_grade' ) {
		...
		$t->set_var('ADD_GRADE_LINK',"<a href=\"scaleinput.php?space_key=$space_key&module_key=$module_key&scale_key=$scale_key&action=select_scale&grade_action=add_grade\">".$gradebook_strings['add_grade'].'</a>');		
