...
* @version $Id: search.php,v 1.6 2007/07/30 01:57:03 glendavies Exp $
* 
*/
...
if ($rs->EOF) {
	...
	while (!$rs->EOF) {
		...
		if ($status_key==2 || (isset($_SESSION['current_user_key']) && $added_by_key==$_SESSION['current_user_key'])) {
			...
			$entries_array[$n] = $summary_fields['fields'];
			$n++;
	...
	$t->set_var('ELSBlock','');			
	foreach ($entries_array as $value) {
	
		$t->set_var('SUMMARY_FIELDS',$value);
