...
/**
* Page to view a single knowledge base entry
*
...
* @version $Id: entry.php,v 1.18 2007/07/30 01:57:03 glendavies Exp $
* 
*/
...
$t->set_var('ADDED_BY','<a href="'.$CONFIG['PATH'].'/users/userdetails.php?space_key='.$space_key.'&user_key='.$entry_data['added_by_key'].'" target="_new">'.$add_user['first_name'].' '.$add_user['last_name'].'</a>');
...
if ($entry_data['modified_by_key']!=0) {
	...
	$t->set_var('MODIFIED_BY', '<a href="'.$CONFIG['PATH'].'/users/userdetails.php?space_key='.$space_key.'&user_key='.$entry_data['modified_by_key'].'" target="_new">'.$modified_user['first_name'].' '.$modified_user['last_name'].'</a>');
...
if ($is_admin!=true && $entry_data['added_by_key']!=$_SESSION['current_user_key']) {
	...
	$t->set_var('EDIT_STRING',$general_strings['edit']);
	$t->set_var('TEMPLATE_KEY',$entry_data['template_key']);	
		...
		switch ($rs->fields[2]) {
			...
			case 1:
				...
				if ($rs->fields[1]!='') {
					...
					$t->set_var('DATA',$objHtml->parseText($rs->fields[1]));		
			...
			case 3:
				...
				if (is_file($full_file_path)) {
					...
					$file_size = filesize($full_file_path)/1024;
					if ($file_size<1024) {
						$file_size = round($file_size,1).'KB';
					} else {
						$file_size = round($file_size/1024,1).'MB';
					}
					$t->set_var('DATA','<a href="'.$file_path.'">'.$general_strings['download_file'].'</a> ('.$file_name.' '.$file_size.')');
...
if ($rs->EOF) {
	$t->set_block('thread', 'ButtonBlock', 'BttnBlock');
	$t->set_var('BttnBlock','');	
} else {
