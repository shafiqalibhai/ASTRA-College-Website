...
$t->set_file(array(
	'body'			=> 'spaces/rssNewsFetchopml.ihtml'
));
...
if ($space_key == $CONFIG['DEFAULT_SPACE_KEY']) {
	...
	$sql = "SELECT DISTINCT
		...
		ORDER BY {$CONFIG['DB_PREFIX']}news.date_added ASC";
...
if (($rs->EOF && $_SESSION['userlevel_key']!='1' && $accesslevel_key!='1' && $accesslevel_key!='3') || $page_details['type_key']==1) {
	...
	$body=preg_replace("/(<\/?)(\w+)([^>]*>)/e"," ", $body);
	//remove all options from select inputs
    $body = preg_replace( "/<option[^>]*>[^<]*/i", '', $body );
    //replace all tags with their text equivalents
    $body = preg_replace( "/<(\/title|hr)[^>]*>/i", "\n          --------------------\n", $body );
    $body = preg_replace( "/<(h|div|p)[^>]*>/i", "\n\n", $body );
    $body = preg_replace( "/<sup[^>]*>/i", '^', $body );
    $body = preg_replace( "/<(ul|ol|br|dl|dt|table|caption|\/textarea|tr[^>]*>\s*<(td|th))[^>]*>/i", "\n", $body );
    $body = preg_replace( "/<li[^>]*>/i", "\n· ", $body );
    $body = preg_replace( "/<dd[^>]*>/i", "\n\t", $body );
    $body = preg_replace( "/<(th|td)[^>]*>/i", "\t", $body );
    $body = preg_replace( "/<a[^>]* href=(\"((?!\"|#|javascript:)[^\"#]*)(\"|#)|'((?!'|#|javascript:)[^'#]*)('|#)|((?!'|\"|>|#|javascript:)[^#\"'> ]*))[^>]*>/i", "[LINK: $2$4$6] ", $body );
    $body = preg_replace( "/<img[^>]* alt=(\"([^\"]+)\"|'([^']+)'|([^\"'> ]+))[^>]*>/i", "[IMAGE: $2$3$4] ", $body );
    $body = preg_replace( "/<form[^>]* action=(\"([^\"]+)\"|'([^']+)'|([^\"'> ]+))[^>]*>/i", "\n[FORM: $2$3$4] ", $body );
    $body = preg_replace( "/<(input|textarea|button|select)[^>]*>/i", "[INPUT] ", $body );
    //strip all remaining tags (mostly closing tags)
    $body = strip_tags( $body );
    //convert HTML entities
    $body = strtr( $body, array_flip( get_html_translation_table( HTML_ENTITIES ) ) );
    preg_replace( "/&#(\d+);/me", "chr('$1')", $body );
    //wordwrap
    $body = wordwrap( $body );
