...
$email_domain = 'dumb.school.nz';
$fcontents = file ('/etc/passwd');
...
while (list ($line_num, $line) = each ($fcontents)) {
	...
	$name		 	= explode(' ',$user_array[4]);
	$first_name  	= $name[0];
	$last_name	 	= $name[1];
	$email 			= $username.'@'.$email_domain;
	
	if ($user_id>500 && $user_id<65000 && strpos($username, '$')===false) {
	
		//echo "Add adding $username,  $first_name $last_name $email<br />";
		$user_data = array('username' => $username, 'first_name' => $first_name,'last_name' => $last_name,'email' => $email);
