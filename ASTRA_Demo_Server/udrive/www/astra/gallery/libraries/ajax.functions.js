 ...
 */
	...
	function updateGalleriesSelect(request)
	{
		...
		if (result != "null") {
			...
			menu += '<select onchange="setGallery(this.value,false)">';
	...
	{
		if (compact_navigation && (id_files[id] > th_diff)) {
	...
	function updateImage(request)
	{
		...
		if (resized_img && create_thumbnails) {
			imgout = '<a href="'+escapedImg+'" target="_blank">';
	...
	function checkNavigation(num)
	{
		...
		if (continuous_nav)
			i_next = ((n+th_min)<id_files[active_id])
				? 'inline'
				: 'none';
